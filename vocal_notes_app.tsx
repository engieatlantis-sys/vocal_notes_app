...
import { Note, NewNote } from './src/types';
import { getAllNotes, setNote as storageSetNote, deleteNoteById } from './src/storage';
...
const VocalNotesApp = () => {
  ...
  const simulateVoiceRecording = async () => {
    ...
    timeoutRef.current = window.setTimeout(async () => {
      ...
      try {
        // IMPORTANT: Appel API côté serveur pour sécuriser la clé API
        ...
        setNewNote({
          ...
          notificationDate: data.priority === 'urgente' 
        ...
        // Fallback: afficher la note brute sans analyse IA
        setNewNote({
          title: 'Nouvelle note',
  ...
  const saveNote = async () => {
    ...
    try {
      await storageSetNote(note as Note);
      await loadNotes();
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
      ...
      <div className="px-6 py-4">
        ...
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full bg-slate-800/50 border border-slate-700 rounded-xl pl-10 pr-4 py-3 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          <button
            ...
            className={`px-4 py-2 rounded-lg whitespace-nowrap transition ${activeCategory === 'all' ? 'bg-blue-500 text-white' : 'bg-slate-800 text-slate-300'}`}
          >
            Tout ({notes.length})
          </button>
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-2 rounded-lg whitespace-nowrap flex items-center gap-2 transition ${activeCategory === cat.id ? `${cat.color} text-white` : 'bg-slate-800 text-slate-300'}`}
            >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <h3 className={`font-semibold ${note.completed ? 'line-through opacity-60' : ''}`}>
                        {note.title}
                      </h3>
                      <button
                    ...
                    <p className={`text-sm text-slate-400 mb-3 ${!isExpanded && 'line-clamp-2'}`}>
                      {note.content}
                    </p>
                    <div className="flex items-center gap-2 text-xs text-slate-500 mb-3">
                      ...
                      {note.hasNotification && (
                        <span className="flex items-center gap-1 text-yellow-400">
                          ...
                          {new Date(note.notificationDate).toLocaleDateString('fr-FR')}
                        </span>
                      )}
                    </div>
                    {isExpanded && (
                      <div className="flex gap-2">
                        {note.category === 'tache' && (
                          <button
                            ...
                            className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium transition ${note.completed ? 'bg-slate-700 text-slate-400' : 'bg-green-500/20 text-green-400 border border-green-500/30'}`}
                          >
                            {note.completed ? <><X className="w-4 h-4 inline mr-1" />Réouvrir</> : <><Check className="w-4 h-4 inline mr-1" />Terminer</>}
                          </button>
                        )}
                        <button
                          onClick={() => editNote(note)}
                          className="flex-1 bg-blue-500/20 text-blue-400 border border-blue-500/30 py-2 px-3 rounded-lg text-sm font-medium hover:bg-blue-500/30 transition"
                        >
                        ...
                        <button
                          ...
                          className="flex-1 bg-red-500/20 text-red-400 border border-red-500/30 py-2 px-3 rounded-lg text-sm font-medium hover:bg-red-500/30 transition"
                        >
      ...
      <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2">
        <button
          ...
          className={`${isRecording ? 'bg-red-500 animate-pulse' : 'bg-blue-500 hover:bg-blue-600'} text-white p-6 rounded-full shadow-2xl transition-all transform hover:scale-105 active:scale-95 disabled:cursor-not-allowed`}
        >
          <Mic className="w-8 h-8" />
        </button>
        {isRecording && (
          <div className="absolute -top-16 left-1/2 transform -translate-x-1/2 bg-slate-800 px-4 py-2 rounded-full text-sm whitespace-nowrap">
            Écoute en cours...
          </div>
        )}
      </div>
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {categories.map(cat => (
                    <button
                      ...
                      className={`p-3 rounded-lg flex flex-col items-center gap-2 transition ${newNote.category === cat.id ? `${cat.color} text-white` : 'bg-slate-900 text-slate-400'}`}
                    >
              ...
              <div>
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={newNote.hasNotification}
                    onChange={(e) => setNewNote({...newNote, hasNotification: e.target.checked})}
                    className="w-5 h-5 rounded"
                  />
                  <span className="text-sm font-medium">Ajouter une notification</span>
                </label>
              </div>
              {newNote.hasNotification && (
                <div>
                  ...
                  <input
                    type="datetime-local"
                    value={newNote.notificationDate}
                    onChange={(e) => setNewNote({...newNote, notificationDate: e.target.value})}
                    className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
              )}
              <button
                ...
                className="w-full bg-blue-500 hover:bg-blue-600 disabled:bg-slate-700 disabled:cursor-not-allowed text-white py-3 rounded-lg font-medium transition"
              >
